...
\end{equation}
The magnetic flux can change in two ways, it can vary temporally with $S$ held fixed or by the advection of the plasma bounded by $C$. These changes are mathematically represented by,
\begin{equation}
...
%--------------------------------------------
Spicules are thought to be able to act as waveguides in the solar atmosphere, and the dynamics that we encounter later into this thesis may have wave-based origins. To understand the basic wave behavior of an ideal plasma, we first consider a homogeneous plasma with no gravity, with a uniform vertical magnetic field $\bs{B}=(0,0,B_0)$, in pressure equilibrium, with no initial flow. To study the waves in this medium we first linearise the ideal MHD equations to study small perturbations (subscript $1$) from the plasma equilibrium (subscript $0$),
...
\end{equation}
is the tube speed. If we don't consider a special case then each solution gives for \eqref{MA_waves} gives the forward and backwards fast (taking the $+$ of the last term) and slow (taking the $-$ of the last term) magnetoacoustic (MA) waves. Hence, these wave modes are a combination of magnetic and sound waves, and therefore the restoring forces are a combination of both pressure and Lorentz force. As these waves have mixed properties, it illustrates that the wave behaviour is heavily affected by $\beta$. If we consider a special case of $\beta \gg 1$, hence can omit any magnetic terms, i.e. $V_{A0}=0$, then one obtains the forward and backward propagating sound waves from \eref{MA_waves}, $\omega = \pm k c_{s0}$. The sound waves are only possible if the fluid is compressible and they are longitudinal waves. If the environment is uniform in temperature then they will propagate isotropically. While these systems of equations give a description of a homogeneous medium, these waves cause different behaviors when confined inside a waveguide.
%---------------------------------------------
...
\begin{equation}
    m_{i,e}^2 = \frac{(k_z^2 c_{si,se}^2-\omega^2)(k_z^2 V_{Ai,Ae}^2-\omega^2)}{(c_{si,se}^2+V_{Ai,Ae}^2)(k_z^2 c_{Ti,Te}^2-\omega^2)}.
\end{equation}
The solutions of \eref{eva_trapp} have different functional form depending on the sign of $m^2_{i,e}$. Solutions with $m^2_{i,e}>0$ are trapped by the waveguide and are known as trapped modes. Whereas with $m^2_{i,e}<0$ leak some wave energy laterally and are known as leaky modes. As we are not interested in searching for solutions for the leaky waves, we focus on the trapped waves, which have a solution of the form,
\begin{equation}
        B_0(x) = 
    \begin{cases}
